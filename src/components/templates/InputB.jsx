    ...
    return (
        <>
            <div className={`${props.classNameDiv?? ''}`} id={`${props.idDiv?? ''}`} name={`${props.nameDiv?? ''}`}>
                ...
                <Field  as={`${props.as}`} type={`${props.typeInput}`}  name={`${props.nameInput ?? ''}`} id={`${props.idInput ?? ''}`} placeholder={`${props.placeholderInput ?? ''}`} className={`${props.classNameInput?? ''}`} autoComplete={`${props.autoCompleteInput?? ''}`} disabled={props.disabledInput?? ''} minLength={props.minLengthInput?? ''} maxLength={props.maxLengthInput?? ''} innerRef={props.innerRef} onKeyUp={props.onKeyUpInput} onKeyDown={props.onKeyDownInput} onClick={props.onClickInput}>
                    {props.children&&props.children}
                </Field>
                {props.ErrorMessage&&<div className={`${props.classNameError?? ''}`}><ErrorMessage name={`${props.nameInput}`} component='div'/></div>}
            </div>
        </>
    )
}
